...
describe('release metadata', () => {
    ...
    test('keeps the current release focused on Campaign Approval Tracking, Campaign History, and current fixes', () => {
        ...
        try {
            ...
            expect(items[4].textContent).toContain('Loading Facts');
            ...
            expect(items[7].textContent).toContain('New Prisma Address');
            expect(items[7].querySelector('.release-badge').dataset.releaseType).toBe('improved');
            expect(items[8].textContent).toContain('four campaigns per page');
            expect(items[9].textContent).toContain('Campaign Approval Tracking');
            expect(items[10].textContent).toContain('Moe Chat');
            expect(items[11].textContent).toContain('Features Off');
            expect(items[12].textContent).toContain('Help Guides');
            expect(items[13].textContent).toContain('Prisma Sign-in Assistant');
            expect(items[14].textContent).toContain('Campaign Name Copy');
            expect(items[15].textContent).toContain('Plan Navigation');
            expect(items[16].textContent).toContain('Not Submitted');
            expect(items[17].textContent).toContain('already-approved campaign');
            expect(items[18].textContent).toContain('clear idle state');
            expect(items[19].textContent).toContain('Redistribute action');
            expect(items[21].textContent).toContain('permanent Moe chat bubble');
            expect(items[22].textContent).toContain('retired Loading Fact Review');
            expect(items[5].textContent).toContain('Getting Started');
            expect(items[6].textContent).toContain('Feature Previews');
        } finally {
            dom.window.close();
        }
    });
