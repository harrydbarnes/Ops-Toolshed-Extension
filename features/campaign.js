(function() {
    ...
    function formatCompactCurrency(value) {
        ...
        if (number >= 1000000) {
            // e.g. 1.1m (one decimal if not whole)
            return symbol + (number / 1000000).toFixed(number % 1000000 === 0 ? 0 : 1) + 'm';
        }
        if (number >= 1000) {
            // e.g. 60k (no decimals)
            return symbol + (number / 1000).toFixed(0) + 'k';
        }

        return symbol + number;
    }

    function handleBudgetDisplay() {
        if (campaignNavStyle !== 'new' || !optimisedNewNavEnabled) return;
