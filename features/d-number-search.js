(function() {
    ...
    async function handleDNumberSearch(dNumber) {
        ...
        try {
            ...
            // Using a shorter timeout here as the overlay should appear quickly after the first delay
            ...
            // 3. (Optional but good practice) Ensure D-Number is on the clipboard
            ...
            // --- NEW Robust Input Strategy: Manually set value ---
            
            // 4. Force the value into the field and dispatch events (bypassing unreliable document.execCommand('paste'))
            searchInput.focus();
            searchInput.value = dNumber;
            console.log(`[DNumberSearch] Manually set input value to: ${dNumber}`);

            // 5. Dispatch ALL possible relevant events to ensure the framework registers the change and starts filtering.
            // Dispatching paste, input, and change events.
            searchInput.dispatchEvent(new Event('paste', { bubbles: true, composed: true }));
