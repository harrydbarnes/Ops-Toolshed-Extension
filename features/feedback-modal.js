class FeedbackModal {
    ...
    open() {
        ...
        this.step = 1;
        this.render();
        // Slight delay to allow DOM render before calculating initial height
        requestAnimationFrame(() => this.updateUI());
    }

    close() {
        if (this.root) {
            // Optional: Animate out before removing could go here
            this.root.remove();
            this.root = null;
        }
    }
...
${this.data.name}`;
    ...
    updateUI() {
        ...
        if (this.step === 1) {
            ...
            // Animate Height
            viewport.style.height = `${step1.offsetHeight}px`;
            ...
            // Slide to -50% (since width is 200%, -50% is the second half)
            track.style.transform = 'translateX(-50%)'; 
            step1.classList.remove('active');
            step2.classList.add('active');

            // Animate Height
            viewport.style.height = `${step2.offsetHeight}px`;
