(function() {
    ...
    const defaults = { helpGuidesEnabled: true, bannerUsernameEnabled: true, quickCampaignActionsEnabled: true, campaignNameQuickCopyEnabled: true, campaignHeaderQuickCopyEnabled: true, campaignDateShortcutEnabled: true, budgetWidgetOptimisedEnabled: true, optimisedNewNavEnabled: true, ordersShortcutEnabled: true, actualiseShortcutEnabled: true, actualiseNavbarEnabled: true, approverWidgetPlacementEnabled: true, approverWidgetOptimiseEnabled: true, gmiChatShortcutEnabled: true, addCampaignShortcutEnabled: true };
    ...
    function buildChapters() {
        ...
        const workspaceFeatures = enabledFeatures([
            ['quickCampaignActionsEnabled', 'Campaign actions beside the campaign name'], ['campaignNameQuickCopyEnabled', 'One-click campaign-name copy'], ['campaignHeaderQuickCopyEnabled', 'One-click identifier copy'], ['campaignDateShortcutEnabled', 'Direct campaign-date access'], ['budgetWidgetOptimisedEnabled', 'A compact budget summary'], ['optimisedNewNavEnabled', 'A clearer campaign navigation'], ['ordersShortcutEnabled', 'Direct Orders access'], ['actualiseShortcutEnabled', 'Direct Actualise access'], ['actualiseNavbarEnabled', 'Campaign navigation retained in Actualise']
        ]);
        const supportFeatures = enabledFeatures([
            ['approverWidgetPlacementEnabled', 'Approver controls beside your campaign'], ['approverWidgetOptimiseEnabled', 'Cleaner recipient management'], ['gmiChatShortcutEnabled', 'A campaign-aware GMI support shortcut'], ['addCampaignShortcutEnabled', 'A faster Add Campaign route']
        ]);
        return [
            ...
            { id: 'workspace', title: 'The campaign workspace keeps repeat work close.', description: 'This chapter brings the most useful navigation and header shortcuts together instead of walking you through each control one by one.', features: workspaceFeatures, target: isEnabled('optimisedNewNavEnabled') ? 'campaignNavigation' : 'campaignHeader', fallbackTarget: 'campaignHeader', missingStatus: 'Waiting for the campaign workspace to finish loading…' },
            { id: 'support', title: 'Approvals and support stay in context.', description: 'Campaign-aware tools are kept near the work so you can move from a question to an action without losing your place.', features: supportFeatures, target: isEnabled('approverWidgetPlacementEnabled') || isEnabled('approverWidgetOptimiseEnabled') ? 'approverWidget' : 'campaignWorkspace', fallbackTarget: 'campaignWorkspace', missingStatus: 'Waiting for the support tools to finish loading…' },
