...
def run(playwright):
    ...
    page.add_init_script("""
        window.chrome = {
            ...
            storage: {
                sync: {
                    get: (keys, callback) => {
                        // Mock data including our new toggle
                        const data = {
    ...
    # Check for new toggle text
    page.wait_for_selector('text=Translucent \'AppLearn\' Logo')

    # Check for the new ID
    page.wait_for_selector('#appLearnReplaceToggle', state='attached')

    print("Verified: Found 'Translucent AppLearn Logo' text and #appLearnReplaceToggle")
